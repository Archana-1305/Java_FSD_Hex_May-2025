...
public interface AttendanceRecordRepository extends JpaRepository<AttendanceRecord, Integer>{
	...
	boolean existsByEmployeeIdAndYearAndMonth(int id, int year, int month);

	@Query("Select ar from AttendanceRecord ar where ar.employee.id=?1 and ar.month=?2 and ar.year=?3")
	AttendanceRecord getByEmployeeIdAndMonthAndYear(int id, int month, int year);
