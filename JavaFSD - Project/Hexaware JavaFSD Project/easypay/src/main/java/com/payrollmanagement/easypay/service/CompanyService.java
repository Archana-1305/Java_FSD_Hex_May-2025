...
public class CompanyService {
	...
	// To fetch all Companies with details
	public List<Company> getAll() {		
		return companyRepository.findAll();
	}

	// To fetch all States(from enum)
	public List<?> getAllStates() {
		
		// Create a list to hold the States
		 List<String> states = new ArrayList<>();
		 
		    // Loop through each value of 'States' in the enum
		    for (States s : States.values()) {
		    	
		    	// Add the State to the list as a string
		        states.add(s.name());
		    }
	...
	// To fetch all Cities(from enum)
	public List<?> getAllCities(String state) {
		
		// Create a list to hold the States
		 List<String> cities = new ArrayList<>();
		    try {
		    	
		    	// Convert the input state which is in String Datatype to the corresponding enum value
		        States st = States.valueOf(state);
		        for (City c : City.values()) {
		        	//getState() method returns a States enum
		            if (c.getState() == st) {
		                cities.add(c.name());
		            }
		        }
