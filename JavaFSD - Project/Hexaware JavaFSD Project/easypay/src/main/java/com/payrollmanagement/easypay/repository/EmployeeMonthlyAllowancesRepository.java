...
public interface EmployeeMonthlyAllowancesRepository extends JpaRepository<EmployeeMonthlyAllowances, Integer> {
	...
	@Query("SELECT e FROM EmployeeMonthlyAllowances e WHERE e.employeeId = ?1 AND e.month = ?2 AND e.year = ?3")
EmployeeMonthlyAllowances getByEmployeeIdAndMonthAndYear(int employeeId, int month, int year);
