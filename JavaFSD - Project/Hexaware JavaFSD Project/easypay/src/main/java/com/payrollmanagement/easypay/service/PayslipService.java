...
//        }
    ...
    public List<MonthlyTotalNetPayDto> getApprovedMonthlyNetPayStats() {
        List<Payslip> approvedPayslips = payslipRepository.findByStatus(PayslipStatus.APPROVED);
        List<MonthlyTotalNetPayDto> stats = new ArrayList<>();
        ...
        for (Payslip payslip : approvedPayslips) {
            ...
            boolean found = false;
            for (MonthlyTotalNetPayDto dto : stats) {
                if (dto.getYear() == year && dto.getMonth() == month) {
                    dto.setTotalNetPay(dto.getTotalNetPay() + netPay);
                    found = true;
                    break;
                }
            }
            if (!found) {
                stats.add(new MonthlyTotalNetPayDto(year, month, netPay));
            }
        }
    ...
    public List<Payslip> getAllPayslips(String username) {
        return payslipRepository.getAllPayslips(username);
    }




	public List<Payslip> getByPayrunAndDepartment(int departmentId, int payrunId) {
		return payslipRepository.getByPayrunAndDepartment(departmentId,payrunId);
	...
	public List<Payslip> getAllPayslipsByStatus(PayslipStatus status) {
		return payslipRepository.getAllPayslipsByStatus(status);
