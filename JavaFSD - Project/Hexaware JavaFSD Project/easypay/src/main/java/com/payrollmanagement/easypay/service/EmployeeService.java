...
public class EmployeeService {
    ...
    public List<EmployeeDto> getEmployeesByCompanyId(int companyId) {
        ...
        List<Employee> list = employeeRepository.findByCompanyId(companyId);

        return EmployeeDto.convertEmployeeListToDto(list);
    }
    // Update
	...
	public Employee insertEmployee(int designationId,Employee employee) {
		 // Extract and configure user from employee
	    User user = employee.getUser();
	  //  user.setRole("EMPLOYEE");
	    user = userService.signUp(user); // Save user first
