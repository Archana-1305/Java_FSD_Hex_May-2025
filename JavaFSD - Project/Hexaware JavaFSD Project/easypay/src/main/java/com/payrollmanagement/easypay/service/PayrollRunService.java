...
//2. Revise a specific payslip
  public void revisePayslip(int payslipId) {
      Payslip payslip = payslipRepository.findById(payslipId)
          .orElseThrow(() -> new ResourceNotFoundException("Payslip not found"));
      if (payslip.getStatus() != PayslipStatus.REVISE && payslip.getStatus() != PayslipStatus.ON_HOLD) {
          throw new IllegalStateException("Payslip is not eligible for revision.");
      }
      // Check attendance, regenerate payslip, etc.
      payslipService.regeneratePayslip(payslip);
  }

public List<PayrollRun> getAllPayruns() {
