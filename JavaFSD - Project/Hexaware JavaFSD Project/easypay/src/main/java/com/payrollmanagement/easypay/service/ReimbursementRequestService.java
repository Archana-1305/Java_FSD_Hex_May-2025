...
public class ReimbursementRequestService {
	 ...
	 private  ReimbursementRequestRepository reimbursementRequestRepository;
		...
		public ReimbursementRequestService(ReimbursementRequestRepository reimbursementRequestRepository,
				...
				MonthReimbursementService monthReimbursementService) {
			super();
			...
			this.monthReimbursementService = monthReimbursementService;
		}
	    ...
	    public ReimbursementRequest updateStatusAndAmount(int requestId, RequestStatus status, Double amount) {
	        ...
	        reimbursementRequestRepository.save(req);

	        // --- Auto-update MonthReimbursement if approved ---
	        if (status == RequestStatus.APPROVED) {
	            monthReimbursementService.calculateAndSaveTotalReimbursement(
	                req.getEmployee().getId(), req.getMonth(), req.getYear()
	            );
	        }
		 ...
		 public List<ReimbursementRequest> getReimbursementsByStatus(RequestStatus status) {
        return reimbursementRequestRepository.getReimbursementsByStatus(status);
    }
